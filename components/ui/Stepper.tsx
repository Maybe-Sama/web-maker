...
export default function Stepper({
  ...
  const { onClick: customOnClick, ...otherNextButtonProps } = nextButtonProps;

  const handleNext = (e?: React.MouseEvent<HTMLButtonElement>) => {
    // Si se pasa una función onClick personalizada, ejecutarla primero
    if (customOnClick && e) {
      customOnClick(e);
    }
    
    // Si el botón está disabled, no proceder
...
function StepIndicator({
  ...
  return (
    ...
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: "#e0e0e0", color: "#666" },
